    ...
    Citizen.CreateThread(function()
        Framework = GetFramework()
            ...
            AddEventHandler('esx:playerLoaded', function(src)
                Wait(1000)
                print(GetPlayerIdentifier(src, 1))
    ...
    if Config.Framework == "OLDQBCore" then
        ...
        function giveitem(Give)
            ...
            if Give.type == 'car' then 
                ...
                vehicleData.model = GetHashKey(Give.item)
                vehicleData.plate = plate
                ExecuteSql("INSERT INTO player_vehicles (license, citizenid, plate, vehicle, status, state) VALUES ('"..Framework.Functions.GetIdentifier(source, 'license').."', '"..Player.PlayerData.citizenid.."', '"..plate.."', '"..json.encode(vehicleData).."', 'car', 0)")
            end
            if Give.type == 'item' then 
...
function GetAvatar(source)
    ...
    if steamhex ~= nil and steamhex ~= '' then
        ...
        local discord = GetPlayerIdentifier(source, 2)
        sub = string.sub(discord, 9)
        PerformHttpRequest('https://discordlookup.mesavirep.xyz/v1/'..sub, function(a,b,c)
