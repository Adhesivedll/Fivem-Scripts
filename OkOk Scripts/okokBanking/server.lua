...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Config.IBANChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerEvent('okokBanking:AddTransferTransactionToSociety', amount, source, "bank", "Bank (IBAN)")
		TriggerClientEvent('okokBanking:updateIban', source, iban)
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "IBAN successfully changed to "..iban, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "You need to have "..Config.IBANChangeCost.."€ in order to change your IBAN", 5000, 'error')
	end
end)
